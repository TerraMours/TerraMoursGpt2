...
export function useSignalR(hubUrl: string) {
  ...
  const imgUrl = ref<ApiGptManagement.ImageRes[]>([]);
  const ms = useMessage();
    ...
    if (connection.value) {
      ...
      connection.value
        ...
        .then(() => {
          ...
          connection.value?.invoke('GetWaitingCount').then(count => {
          ...
          connection.value?.on('updateImgUrl', newImgUrl => {
            ...
            if (typeof newImgUrl === 'string') {
              ...
              ms.success('图片生成成功。');
              imgUrl.value = newImgUrl.map((imagUrl: string) => ({ imagUrl })) as ApiGptManagement.ImageRes[];
            }
          });
        })
        .catch(error => {
  ...
  const getConnection = (hubUrl: string): HubConnection => {
